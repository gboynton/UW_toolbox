function ms=mseq(nTypes,powerVal,shift,whichSeq)
%Maximum length sequence assuming 2,3,5 distinct values
%
%[ms]=MSEQ(nTypes,powerVal,[shift,whichSeq])
%
%
%INPUT:  
%           nTypes: number of stimulus types
%           powerVal:  determines length of sequence: 2^powerVal - 1
%           shift:  circular shift parameter. Default is 0;
%           whichSeq: determines which of a finite number of m-sequences  to be calculated for
%               a given set of nTypes and powerVals.  Default is 1.
%       
...
% Edited by SNL-B on 3/30/05

if nTypes == 4 | nTypes ==1 | nTypes > 5
    error(sprintf('Can only use 2,3 or 5 stimulus conditions (not %d)',nTypes));
    ms = [];
    return
end
...
if nTypes==2
    switch powerVal
        ...
        case 30
            tap(1).No=[7,28,29,30];
        otherwise error(sprintf('M-sequence %.0f^%.0f length is not defined',nTypes,powerVal))
    end
elseif nTypes==3
    switch powerVal
        case 2
        ...
        otherwise error(sprintf('M-sequence %.0f^%.0f is not defined',nTypes,powerVal))
    end
elseif nTypes==5
    switch powerVal
        case 2 
...
if isempty(whichSeq)
    ...
    if whichSeq>length(tap) | whichSeq<1
        ...
        disp(sprintf('%d events with power of %d only allows %d types. wrapping around to %d.',nTypes,powerVal,length(tap),whichSeq));
    end
end
...
for i=1:bitNum
    ...
    %   register, (tap(1).No)
    ms(i)=rem(weights*register+nTypes,nTypes);
